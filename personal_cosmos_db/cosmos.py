...
def runDemo(writeOutput):
    # load_dotenv()
    # Get the root directory (one level up from this file)
    root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    env_path = os.path.join(root_dir, "localsettings.env")
    load_dotenv(dotenv_path=env_path)

    # Get the connection string from the environment variable
    connection_string = os.getenv("PERSONAL_COSMOSDB_CONNECTION_STRING")
    if not connection_string:
        raise ValueError("PERSONAL_COSMOSDB_CONNECTION_STRING not set in localsettings.env")
